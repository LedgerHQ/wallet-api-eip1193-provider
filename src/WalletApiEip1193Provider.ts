...
import {
  getSupportedCurrencyIds,
  getChainConfigForChainId,
  getChainConfigForCurrencyId,
} from "./chainConfig";
import { convertEthToLiveTX, matchAddresses } from "./helpers";
import axios from "axios";
import { v4 as uuidv4 } from 'uuid';

export class WalletApiEIP1193Provider
  extends EventEmitter
  implements IEthereumProvider
{
  walletInstance: WalletAPIClient | null = null;
  ...
  enable(): Promise<ProviderAccounts> {
    return this.request({ method: "enable" }) as Promise<ProviderAccounts>;
  }
    ...
    switch (method) {
      ...
      case "wallet_switchEthereumChain": {
        ...
        const [{ chainId: hexChainId }] =
          switchEthereumChainParams.parse(params);
      ...
      case "eth_sendTransaction": {
        ...
        if (!matchAddresses(transaction.from, this.account.address)) {
          throw new Error(
            "Transaction doesn't originate from selected Account"
          );
        }

        const txHash = await this.walletInstance.signTransactionAndBroadcast(
          this.account.id,
          convertEthToLiveTX(transaction),
        );
        return txHash;
      }
      ...
      case "personal_sign": {
        ...
        const signedMessage = await this.walletInstance.signMessage(
          this.account.id,
